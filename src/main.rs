...
async fn exec_fcgi(fcgi_app: &str) -> std::io::Result<()> {
    ...
    fcgi_request(&mut fcgi, "map=test/helloworld.qgs&SERVICE=WMS&REQUEST=GetCapabilities");
    // fcgi_request(&mut fcgi, "map=test/helloworld.qgs&ERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&BBOX=-67.593,-176.248,83.621,182.893&CRS=EPSG:4326&WIDTH=515&HEIGHT=217&LAYERS=Country,Hello&STYLES=,&FORMAT=image/png;%20mode%3D8bit&DPI=96&TRANSPARENT=TRUE");
